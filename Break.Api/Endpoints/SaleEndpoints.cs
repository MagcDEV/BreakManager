...
public static class SaleEndpoints
{
    ...
    {
        app.MapPost(ApiEnpoints.Sale.CreateSale, async (CreateSaleRequest request, ISaleService saleService) =>
        {
            var saleItems = request.MapToSaleItems().ToList();
            var sale = await saleService.CreateSaleAsync(saleItems, request.CouponCode);
            
            return Results.Created($"/sales/{sale.SaleId}", new { SaleId = sale.SaleId, Total = sale.Total });
        })
        .WithName("CreateSale");

        app.MapGet(ApiEnpoints.Sale.GetSale, async (int id, ISaleService saleService) =>
        {
        ...
        app.MapGet(ApiEnpoints.Sale.GetAllSales, async (ISaleService saleService) =>
        {
